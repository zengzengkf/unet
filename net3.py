...
class UNet(nn.Module):
    def __init__(self,base_channel=32):
        ...
        self.enc1 = nn.Sequential(
            DenseBlock(base_channel,base_channel//4,4,max_dropout_rate=0.02),
            HWDownsampling(base_channel*2, base_channel*2)
        )

        self.enc2 = nn.Sequential(
            DenseBlock(base_channel*2,base_channel//4,8,max_dropout_rate=0.03),
            HWDownsampling(base_channel*4, base_channel*4)
        )

        self.enc3 = nn.Sequential(
            DenseBlock(base_channel*4,base_channel//4,16,max_dropout_rate=0.04),
            HWDownsampling(base_channel*8, base_channel*8)
        )

        self.enc4 = nn.Sequential(
            DenseBlock(base_channel*8,base_channel//2,16,max_dropout_rate=0.05),
            HWDownsampling(base_channel*16, base_channel*16)
        )

        # 瓶颈层
        self.bottleneck = nn.Sequential(
            nn.Conv2d(base_channel*16, base_channel*8, kernel_size=1, padding=0),
            nn.BatchNorm2d(base_channel*8),
            nn.ReLU(inplace=True),
            DenseBlock(base_channel * 8, base_channel // 2, 16, max_dropout_rate=0.08),

            nn.Conv2d(base_channel * 16, base_channel * 8, kernel_size=1, padding=0),
            nn.BatchNorm2d(base_channel * 8),
            nn.ReLU(inplace=True),
            DenseBlock(base_channel * 8, base_channel // 2, 16, max_dropout_rate=0.1)
        )
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(base_channel * 8, base_channel * 4, kernel_size=1, padding=0),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(base_channel * 4, base_channel*2 , kernel_size=1, padding=0),
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(base_channel * 2, base_channel , kernel_size=1, padding=0),
            nn.BatchNorm2d(base_channel ),
            nn.ReLU(inplace=True),
            DenseBlock(base_channel , base_channel // 4, 4, max_dropout_rate=0.02),
            nn.Conv2d(base_channel * 2, base_channel, kernel_size=1, padding=0),
